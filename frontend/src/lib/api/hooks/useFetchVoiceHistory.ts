import { USER_GENERATED_VOICES } from '@/lib/constants';
import { elevenlabsRequestHeaders } from '@/lib/constants/api';
import { TAudioVersions } from '@/lib/db/schema';
import { useAppDispatch } from '@/redux/hooks';
import { useQuery } from '@tanstack/react-query';
import axios, { AxiosError } from 'axios';
import { _1Min } from '../../../redux/constants/time';

export const useFetchVoiceHistory = (dependsOn = true) => {
  const dispatch = useAppDispatch();

  const fetchUserGeneratedVoices = async () => {
    const { data } = await axios.get<TAudioVersions[]>('/api/voice-history', {
      headers: elevenlabsRequestHeaders,
    });

    return data;
  };

  function onSuccess(resp?: TAudioVersions[]) {
    if (!!resp?.length) {
      // dispatch(appActions.setUserVoiceHistory(resp!));
    }
  }

  // on Error
  function onError(error: AxiosError) {
    throw error;
  }

  return useQuery({
    queryKey: [USER_GENERATED_VOICES],
    queryFn: fetchUserGeneratedVoices,
    keepPreviousData: true,
    onSuccess,
    onError,
    retry: 0,
    staleTime: 15 * _1Min,
    cacheTime: 20 * _1Min,
    enabled: dependsOn,
  });
};
